...
def validate_ip4(ip_str):
    """
    Validate a binary IPv4 ip address.
    :param ip_str: String to validate as a hexidecimal IPv4 ip address.
    :type ip_str: str
    :returns: ``True`` if a valid hexidecimal IPv4 ip address,
              ``False`` otherwise.
    :raises: TypeError
    '''
    """
...
def validate_domain(domain_str):
    """
    Validate a domain.
    :param domain_str: String to validate as a domain.
    :type domain_str: str
    :returns: ``True`` if a valid domain,
              ``False`` otherwise.
    :raises: TypeError
    '''
    """
    _DOMAIN_RE = re.compile(r'(?i)\b((?=[a-z0-9-]{1,63}\.)(xn--)?[a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,6}\b')
...
if __name__ == '__main__':
    ...
    print(validate_ip4('255.1.1.255'))
